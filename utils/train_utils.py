...
class DataAugmentation(nn.Module):
    ...
    def forward(self, x: Tensor, train_count=None) -> Tensor:
        ...
        if train_count is None:
            ...
            additional_aug = self.randaugmentation.form_transforms(train_count)
            ...
            for i in range(len(x)):
                add_transform = nn.Sequential(*additional_aug[i])
