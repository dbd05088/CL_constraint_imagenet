...
class MemoryDataset(Dataset):
    def __init__(self, dataset, transform=None, cls_list=None, device=None, test_transform=None,
                 data_dir=None, transform_on_gpu=True, save_test=None, keep_history=False, use_kornia=True, buf_transform=None, cls_weight_decay=None, weight_option=None, weight_ema_ratio=None):
        self.use_kornia = use_kornia
        self.datalist = []
    ...
    def register_stream(self, datalist):
        ...
        for data in datalist:
            ...
            '''
            self.stream_image_pathes.append(img_path)
    ...
    def replace_sample(self, sample, idx=None, logit=None, attention_map=None, count=None, task=None):
        ...
        if idx is None:
            ...
            '''
            self.img_pathes.append(img_path)
    ...
    def classwise_get_weight(self, weight_method, batch_size):
        weight = np.zeros(len(self.labels))
        weight = weight.astype('float64')
    ...
    @torch.no_grad()
    def get_batch(self, batch_size, stream_batch_size=0, use_weight=None, transform=None, twf=False, recent_ratio=None, exp_weight=False, use_human_training=False, prev_batch_index=None, weight_method=None):
        assert batch_size >= stream_batch_size
        stream_batch_size = min(stream_batch_size, len(self.stream_image_pathes))
            ...
            if use_weight == "classwise":
                ...
                for place in list(set(selected_place)):
                    ...
                    if len(place_index) > len(klass_index):
                        ...
                        klass_selected_index.extend(random.sample(range(len(self.labels)), len(place_index) - len(klass_index)))
                    else:    
                        # random 기반
                ...
                indices = np.random.choice(range(len(self.labels)), size=memory_batch_size, replace=False, p=weight)
        ...
        if stream_batch_size > 0:
            if len(self.stream_image_pathes) > stream_batch_size:
                stream_indices = np.random.choice(range(len(self.stream_image_pathes)), size=stream_batch_size, replace=False)
            else:
                stream_indices = np.arange(len(self.stream_image_pathes))
        ...
        if self.use_kornia:
            ...
            if stream_batch_size > 0:
                for i in stream_indices:
                    img_path = self.stream_image_pathes[i]
                    if self.use_kornia:
                        images.append(self.preprocess(PIL.Image.open(img_path).convert('RGB')))
                    elif self.transform_on_gpu:
                        images.append(self.transform_cpu(PIL.Image.open(img_path).convert('RGB')))
                    else:
                        images.append(PIL.Image.open(img_path).convert('RGB'))
...
class GdumbMemory(MemoryDataset):
    ...
    def replace_sample(self, sample, idx=None):
        self.cls_count[self.cls_dict[sample['klass']]] += 1
        if idx is None:
            self.cls_idx[self.cls_dict[sample['klass']]].append(len(self.labels))
            self.datalist.append(sample)
            self.usage_cnt.append(0)
            # try:
            #     img_name = sample['file_name']
    ...
    def get_batch(self, batch_size, stream_batch_size=0, use_weight=False, transform=None):
        ...
        if memory_batch_size > 0:
            if use_weight:
                ...
                indices = np.random.choice(range(len(self.labels)), size=memory_batch_size, p=weight / np.sum(weight),
                                           replace=False)
            else:
                indices = np.random.choice(range(len(self.labels)), size=memory_batch_size, replace=False)
        if stream_batch_size > 0:
